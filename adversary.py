...
class TransitionClassifier(object):
    def __init__(self, observation_space, action_space, hidden_size,
        ...
        self.scope = scope
        # self.observation_shape = (np.prod(observation_space.shape),)
        ...
        # print(self.generator_obs_ph.get_shape())
        # print(self.generator_acs_ph.get_shape())
        # Build graph
    ...
    def build_graph(self, obs_ph, acs_ph, reuse=False):
        ...
        with tf.variable_scope(self.scope):
            ...
            if self.discrete_actions:
                ...
                actions_ph = acs_ph
            # print(obs.get_shape(), actions_ph.get_shape())
            # obs = tf.reshape(obs, [-1, obs.get_shape()[1]])
            # actions_ph = tf.reshape(actions_ph, [-1, actions_ph.get_shape()[1]])
            # print(obs.get_shape(), actions_ph.get_shape())
    ...
    def get_reward(self, obs, actions):
        ...
        """
        # obs = np.array([ob.ravel() for ob in obs])
        obs = obs[:,:,:3]
        obs = obs.ravel()
