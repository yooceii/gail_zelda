    ...
    def setup_model(self):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                if self.using_gail:
                    self.obs_space = deepcopy(self.observation_space)
                    self.obs_space.dtype = np.float32
                    shape = self.observation_space.shape
                    shape = shape[:-1] + (3,)
                    self.obs_space.shape = (np.prod(shape),)
                    self.reward_giver = TransitionClassifier(self.obs_space, self.action_space,
            ...
            with self.sess.as_default():
                ...
                while True:
                    ...
                    if self.using_gail:
                        ...
                        lr_local = (seg["ep_lens"], seg["ep_rets"], seg["ep_true_rets"], seg["ep_lvls"])  # local values
                        list_lr_pairs = MPI.COMM_WORLD.allgather(lr_local)  # list of tuples
                        lens, rews, true_rets, lvls = map(flatten_lists, zip(*list_lr_pairs))
                        true_reward_buffer.extend(true_rets)
                    else:
                        # lr: lengths and rewards
                        lr_local = (seg["ep_lens"], seg["ep_rets"], seg["ep_lvls"])  # local values
                        list_lr_pairs = MPI.COMM_WORLD.allgather(lr_local)  # list of tuples
                        lens, rews, lvls = map(flatten_lists, zip(*list_lr_pairs))
                    len_buffer.extend(lens)
                    reward_buffer.extend(rews)
                    lvls_buffer.extend(lvls)

                    if len(len_buffer) > 0:
                        logger.record_tabular("EpLvlsMean", np.mean(lvls_buffer))
